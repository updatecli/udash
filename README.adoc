...
then `$HOME/.udash/`, then `/etc/udash/`. The first one found wins. `--config` takes the path of a
different file instead, which then must exist.
...
gc:
  # maxHistoryDays is how many days of reports are kept, see "Data retention".
...
* **UDASH_GC_MAX_HISTORY_DAYS**: Days of reports to keep, `0` keeps everything
* **UDASH_GC_INTERVAL**: Time between two garbage collections run by the server, such as `24h`
* **UDASH_GC_BATCH_SIZE**: Maximum number of reports deleted by a single statement

==== Data retention

By default Udash keeps every report forever. Setting `gc.maxHistoryDays` enables a garbage
collector, run by the server a minute after it starts and then every `gc.interval`, which deletes:

* every report older than `gc.maxHistoryDays`, including the last report of a pipeline which
  stopped running, so that pipeline disappears from the dashboard;
* then the scms, labels, and source, condition and target configs which no remaining report
  references anymore, and which were not used within the retention period either.

When several Udash instances share a database, a postgres advisory lock ensures only one of them
collects at a time.

The same garbage collection can be run once from the command line, for instance from a scheduled
job, with `--dry-run` to only count what would be deleted. It reads the same configuration file and
environment variables, and `--max-history-days` overrides the retention. Unlike the server, it never
migrates the database schema, so it must run against a database the server already migrated:
